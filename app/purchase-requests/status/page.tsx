...
const chartColors: Record<PurchaseRequestStatus, string> = {
  approval: "#f59e0b",
  unconfirmed: "#94a3b8",
  confirmed: "#6366f1",
  in_progress: "#0ea5e9",
...
function unitPriceOf(r: PurchaseRequest) {
...
export default function PurchaseRequestStatusPage() {
  ...
  function shortcut(kind: string) {
    ...
    if (kind === "reset") {
      const d = defaultPurchaseRequestDateRange();
      setDateFrom(d.from);
      setDateTo(d.to);
      setDueFrom("");
      setDueTo("");
  ...
  const summary = useMemo(() => summarizePurchaseRequests(filtered), [filtered]);
  ...
  const resultRows: ResultRow[] = useMemo(() => {
    ...
    for (const r of filtered) {
      const mk = monthKey(r.requestDate);
  ...
  const chartData = useMemo(() => {
    return countByPurchaseRequestStatus(filtered)
      .filter((c) => c.status !== "all")
      .map((c) => ({
        name: c.label,
        count: c.count,
        fill: chartColors[c.status as PurchaseRequestStatus],
      }));
  }, [filtered]);
        ...
        <Card className="border-slate-200 shadow-sm">
          <CardContent className="space-y-2 p-3">
            <div className="grid gap-2 lg:grid-cols-2 xl:grid-cols-3">
              ...
              <div className="flex items-stretch overflow-hidden rounded border border-slate-200">
                <span className={labelCls}>발주요청No</span>
                <Input
                  className={cn(fieldCls, "flex-1 rounded-none border-0")}
                  value={slipNo}
