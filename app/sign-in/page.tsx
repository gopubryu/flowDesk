    ...
    <main className="flex min-h-screen items-center justify-center bg-background px-4 py-12">
      <section className="w-full max-w-md rounded-xl border bg-card p-8 shadow-sm">
        <div className="mb-8">
          ...
          <p className="mt-2 text-sm text-muted-foreground">업무 공간에 안전하게 접속하세요.</p>
        </div>
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <label className="block text-sm font-medium">
            비밀번호
            <input className="mt-1.5 w-full rounded-lg border bg-background px-3 py-2.5 text-sm outline-none ring-primary focus:ring-2" type="password" autoComplete="current-password" required value={password} onChange={(event) => setPassword(event.target.value)} />
          </label>
        ...
        <p className="mt-6 text-center text-sm text-muted-foreground">
          계정이 없나요? <Link className="font-semibold text-primary hover:underline" href="/sign-up">회원가입</Link>
        </p>
      </section>
    </main>
  );
