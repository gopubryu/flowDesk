...
import { fetchReceiptSlips } from "@/lib/inventory";
...
export default function ReceiptsListPage() {
  ...
  async function load() {
    try {
      setRows(await fetchReceiptSlips());
    } catch (e) {
      console.error(e);
      await appAlert({
        title: "알림",
        description: e instanceof Error ? e.message : "입고 조회에 실패했어요.",
      });
    } finally {
      setHydrated(true);
    }
  ...
  return (
    <div className="flex h-full flex-col gap-3 p-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-bold text-slate-900">입고조회</h1>
          <p className="text-xs text-muted-foreground">입고전표 목록이에요.</p>
        </div>
        ...
        <table className="w-full text-left text-xs">
          ...
          <tbody>
            {!hydrated ? (
              ...
              <tr><td colSpan={8} className="px-3 py-10 text-center text-muted-foreground">입고전표가 없어요.</td></tr>
            ) : (
              rows.map((r) => (
