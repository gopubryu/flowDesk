...
import { DEMO_WORKSPACE_ID, serializePurchase } from "@/lib/demo";
...
export async function PATCH(req: Request) {
  try {
    ...
    const rows = await serializableInventoryTransaction(async (tx) => {
      const current = await tx.purchase.findMany({ where: { workspaceId: DEMO_WORKSPACE_ID, id: { in: selected } } });
      ...
      await tx.purchase.updateMany({ where: { workspaceId: DEMO_WORKSPACE_ID, id: { in: selected } }, data: { status } });
      return tx.purchase.findMany({ where: { workspaceId: DEMO_WORKSPACE_ID, id: { in: selected }, }, include: { lines: { orderBy: { sortOrder: "asc" } } } });
    });
    return NextResponse.json(rows.map(serializePurchase));
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Could not change purchase status." }, { status: 409 });
  }
}
...
export async function DELETE(req: Request) {
  try {
    ...
    await serializableInventoryTransaction(async (tx) => {
      const current = await tx.purchase.findMany({ where: { workspaceId: DEMO_WORKSPACE_ID, id: { in: selected } } });
      if (current.length !== selected.length) throw new Error("One or more purchases no longer exist.");
      const receiptCounts = await tx.stockMovement.groupBy({ by: ["relatedId"], where: { workspaceId: DEMO_WORKSPACE_ID, relatedType: "purchase", type: "receipt", relatedId: { in: selected } }, _count: { _all: true } });
      ...
      await tx.purchase.deleteMany({ where: { workspaceId: DEMO_WORKSPACE_ID, id: { in: selected } } });
    });
    return NextResponse.json({ ok: true });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Could not delete purchases." }, { status: 409 });
  }
}
