...
import { canDeletePurchase, canTransitionPurchase, calculateImportBaseAmount, finiteNonNegative, validateImportAmounts, validatePurchaseInput } from "@/lib/erp-rules";
...
export async function PATCH(req: Request, ctx: Ctx) {
  try {
    ...
    if (importFieldProvided) {
      ...
      const requestedForeignAmount = body.foreignAmount !== undefined ? body.foreignAmount : existing.foreignAmount?.toString();
      const requestedRate = body.customsExchangeRate !== undefined ? body.customsExchangeRate : existing.customsExchangeRate?.toString();
      const calculatedBaseAmount = calculateImportBaseAmount(body.currency !== undefined ? body.currency : existing.currency, requestedForeignAmount, requestedRate);
      const importValidation = validateImportAmounts({ currency: body.currency !== undefined ? body.currency : existing.currency, foreignAmount: requestedForeignAmount, customsExchangeRate: requestedRate, baseAmount: calculatedBaseAmount ?? (body.baseAmount !== undefined ? body.baseAmount : existing.baseAmount?.toString()), importVatBaseAmount: body.importVatBaseAmount !== undefined ? body.importVatBaseAmount : existing.importVatBaseAmount?.toString(), importVat: body.importVat !== undefined ? body.importVat : existing.importVat?.toString() });
    ...
    if (body.baseAmount !== undefined || body.foreignAmount !== undefined || body.customsExchangeRate !== undefined) data.baseAmount = calculateImportBaseAmount(body.currency !== undefined ? body.currency : existing.currency, body.foreignAmount !== undefined ? body.foreignAmount : existing.foreignAmount?.toString(), body.customsExchangeRate !== undefined ? body.customsExchangeRate : existing.customsExchangeRate?.toString()) ?? body.baseAmount;
