...
import {
  ...
  countByPurchasePlanStatus,
  defaultPurchasePlanDateRange,
  loadPurchasePlans,
...
const chartColors: Record<PurchasePlanStatus, string> = {
  confirmed: "#6366f1",
  in_progress: "#0ea5e9",
  completed: "#22c55e",
};
...
export default function PurchasePlanStatusPage() {
  const defaults = useMemo(() => defaultPurchasePlanDateRange(), []);
  const [rows, setRows] = useState<PurchasePlan[]>([]);
  ...
  function shortcut(kind: string) {
    ...
    if (kind === "reset") {
      const d = defaultPurchasePlanDateRange();
      setDateFrom(d.from);
      setDateTo(d.to);
      setDueFrom("");
      setDueTo("");
  ...
  const summary = useMemo(() => summarizePurchasePlans(filtered), [filtered]);
  ...
  const chartData = useMemo(() => {
    return countByPurchasePlanStatus(filtered)
      .filter((c) => c.status !== "all")
      .map((c) => ({
        name: c.label,
        count: c.count,
        fill: chartColors[c.status as PurchasePlanStatus],
      }));
  }, [filtered]);
  ...
  return (
    <div className="space-y-3">
      ...
      <div>
        <h2 className="text-base font-semibold tracking-tight text-slate-900">발주계획현황</h2>
        ...
        <Card className="border-slate-200 shadow-sm">
          <CardContent className="space-y-2 p-3">
            <div className="grid gap-2 lg:grid-cols-2 xl:grid-cols-3">
              ...
              <div className="flex items-stretch overflow-hidden rounded border border-slate-200">
                <span className={labelCls}>발주계획No</span>
                <Input
                  className={cn(fieldCls, "flex-1 rounded-none border-0")}
                  value={slipNo}
                  ...
                  resultRows.map((r, idx) => {
                    ...
                    return (
                      <tr
                        ...
                        title={PURCHASE_PLAN_STATUS_LABEL[r.status]}
                      >
